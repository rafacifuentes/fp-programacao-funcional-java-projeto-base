...
public class Aula04 extends Aula {
    ...
    protected double maiorNotaCursoAndSexo(@NonNull final Stream<Estudante> stream,
                                           @NonNull final Curso curso,
                                           final char sexo) {

        return stream
                .filter(s -> s.getCurso() != null)
                .filter(s -> s.getCurso().equals(curso))
                .filter(s -> s.getSexo() == sexo)
                .mapToDouble(Estudante::getNota)
                .max()
                .orElse(0);
    }

    protected long totalEstudantesCursoAndSexo(@NonNull final Stream<Estudante> stream,
                                               @NonNull final Curso curso,
                                               final char sexo) {
